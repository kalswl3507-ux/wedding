...
- B 첫 화면은 `B-video.js`가 제어하는 무음 영상입니다. 페이지를 열 때 한 번만 재생하고 12번 원본 사진으로 전환합니다. 사진은 영상 마지막 장면의 인물 위치에 맞춰 CSS로 배치합니다. 원본은 `B_style/Main.mp4`, 배포용은 `assets/B-main.mp4`입니다. 자동재생이 차단되면 영상 재생 버튼을 표시합니다.
