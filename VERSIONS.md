...
- B 첫 화면은 `B-video.js`가 제어하는 무음 영상입니다. 한 번 재생하고 마지막 장면에서 멈추며, 화면 밖으로 나갔다가 다시 보이면 처음부터 재생합니다. 사진으로 전환하지 않습니다. 원본은 `B_style/Main.mp4`, 배포용은 `assets/B-main-hd.mp4`입니다. 자동재생이 차단되면 영상 재생 버튼을 표시합니다.
